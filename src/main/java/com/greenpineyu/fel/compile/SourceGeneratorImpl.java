    ...
    static {
        ...
        try {
            ...
            if (FelSwitcher.errorLog) {
                log.error("SourceGeneratorImpl load resource error", e);
            }
        }
...
//		src = src.replaceAll("\\$\\{expression\\}", expression);
    ...
    private String getClassName() {
        String className;
        synchronized (object) {
            className = "Fel_" + (count++) + "_" + System.currentTimeMillis() + "_" + ThreadLocalRandom.current().nextInt(100);
        }
        return className;
    }
    ...
    public static final Callable<Boolean, FelNode> varsFilter = node -> {
        FelNode n = node[0];
...
//				List<FelNode> repeatNodes = new ArrayList<FelNode>();
                ...
                Map<String, List<FelNode>> repeatNodeMap = new HashMap<>();
                for (FelNode n : nodes) {
